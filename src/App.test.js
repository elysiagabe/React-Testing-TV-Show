...
test('App mounts, fetches initial data, renders to DOM & renders episode list on click', async () => {
    mockFetchShow.mockResolvedValueOnce(mockShowData);
    
    //renders app
    const { getByText, getByTestId, queryAllByTestId } = render (<App />)
    
    //sees fetching data before api call
    getByText(/fetching data/i);
    
    //api call, sees show name
    await waitFor(() => {
        expect(getByTestId(/show-name/i)).toBeInTheDocument;
    })

    //sees & clicks on select a season dropdown
    userEvent.click(getByText(/select a season/i));
    //sees & clicks on season 1 option
    userEvent.click(getByText(/season 1/i));
    //2 episodes render
    expect(queryAllByTestId(/test-episodes/i)).toHaveLength(2);
